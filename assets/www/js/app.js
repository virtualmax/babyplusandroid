...
App = {
    ...
    navInit: function() {
	    ...
	    var std = $(".menu-block:first")
	    $.each($(".menu-block"), function(i, elem) {
		$(elem).height($(std).width());
		$(elem).css("margin-bottom", parseInt($(std).css("margin-right")) - 1);
		$(elem).find("img").css("margin-top", $(std).height()/2 - $(std).find("img").height()/2 - 5);
		setTimeout(function() {
		    $(elem).addClass("on");
		}, 500);
